...
Project scaffold for transformation-source
...
Then, the the generated code will be executed as *`node dist/transformation-source.js`*.  
...
To execute the code, run *`node dist/transformation-source.js`*.  
*Note:* The default Entry point for project is set to `index.ts` and can be modified in `package.json`.
