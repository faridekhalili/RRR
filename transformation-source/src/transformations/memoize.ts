...
export default function memoize(td: TransformationDetails): void {

  const theNode: NodePath<T.Node> = td.astNodeAfter;

  if (!(theNode.isFunction() || theNode.isClass() || theNode.isTaggedTemplateExpression())) {
    logger.log(
      new Log.Builder()
        .withSeverity(LogLevel.LOG)
        .withModule('transformations.memoize()')
  ...
  if (theNode.isFunction()) {
    ...
    if (T.isExpression(theNode.node) || T.isTaggedTemplateExpression(theNode.node)) {
