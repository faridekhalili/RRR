...
export default function importUseRef(td: TransformationDetails): void {
  ...
  if (!theNode.isProgram()) {
    logger.log(
      new Log.Builder()
        ...
        .withModule('transformations.importUseRef()')
  ...
  const importsUseRef: any[] = importDeclarations
  ...
  && (node.specifiers
    .filter((importNode: T.ImportDefaultSpecifier | T.ImportSpecifier): boolean => {
      if (T.isImportDefaultSpecifier(importNode) || T.isImportNamespaceSpecifier(importNode)) {
        ...
        return (<T.Identifier>(<T.ImportSpecifier>importNode).imported).name === 'useRef';
      }
    }).length > 0));

  if (importsUseRef.length === 0) {
    const importSpecifiers: T.ImportSpecifier[] = [T.importSpecifier(T.identifier('useRef'), T.identifier('useRef'))];
    const useRefImport: T.ImportDeclaration = T.importDeclaration(importSpecifiers, T.stringLiteral('react'));
    (<T.Program>theNode.node).body.unshift(useRefImport);
  }
