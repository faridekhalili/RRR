...
export default function importMemo(td: TransformationDetails): void {
  ...
  if (!theNode.isProgram()) {
    logger.log(
      new Log.Builder()
        ...
        .withModule('transformations.importMemo()')
  ...
  const importsMemo: any[] = importDeclarations
  ...
  && (node.specifiers
    .filter((importNode: T.ImportDefaultSpecifier | T.ImportSpecifier): boolean => {
      if (T.isImportDefaultSpecifier(importNode) || T.isImportNamespaceSpecifier(importNode)) {
        ...
        return (<T.Identifier>(<T.ImportSpecifier>importNode).imported).name === 'memo';
      }
    }).length > 0));

  if (importsMemo.length === 0) {
    const importSpecifiers: T.ImportSpecifier[] = [T.importSpecifier(T.identifier('memo'), T.identifier('memo'))];
    const memoImport: T.ImportDeclaration = T.importDeclaration(importSpecifiers, T.stringLiteral('react'));
    (<T.Program>theNode.node).body.unshift(memoImport);
