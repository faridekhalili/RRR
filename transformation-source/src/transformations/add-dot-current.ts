...
export default function addDotCurrent(td: TransformationDetails): void {
  ...
  if (!theNode.isIdentifier()) {
    logger.log(
      new Log.Builder()
        ...
        .withModule('transformations.addDotCurrent()')
  ...
  const memberExpression: T.MemberExpression = T.memberExpression(T.identifier(variableName), T.identifier('current'));
