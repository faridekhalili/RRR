...
export default function useCallbackDep(td: TransformationDetails): void {
  ...
  if (!theNode.isFunction()) {
    logger.log(
      new Log.Builder()
        ...
        .withModule('transformations.useCallbackDep()')
  ...
  const dependencies: Array<T.Identifier> = td.additionalInfo.deps.map((dep: string): T.Identifier => T.identifier(dep));
  const dependencyList: T.ArrayExpression = T.arrayExpression(dependencies);
