...
export default function setStateToAssignmentCurrentValue(td: TransformationDetails): void {
  ...
  if (!theNode.isCallExpression()) {
    logger.log(
      new Log.Builder()
        ...
        .withModule('transformations.setStateToAssignmentCurrentValue()')
  ...
  const lhs: T.MemberExpression = T.memberExpression(
    T.memberExpression(T.identifier(td.additionalInfo.varName), T.identifier('current')),
    T.identifier('value'));
